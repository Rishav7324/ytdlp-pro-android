...
private val entries = listOf(
    LegalEntry("Privacy Policy", "Data, permissions and network requests", Icons.Rounded.PrivacyTip, listOf(
        "Local-first" to "Zyvro does not require a Zyvro account and is designed to keep settings, history and app state on your device.",
        "Network requests" to "When you process a URL, Zyvro and its bundled engines may connect to the requested service. That service may receive your IP address and other request information under its own privacy policy.",
    ...
    LegalEntry("Terms of Use", "Rules for responsible use", Icons.Rounded.Gavel, listOf(
        "Lawful use" to "Use Zyvro only for lawful purposes and only with content you are authorized to access, download, convert or store.",
        ...
        "No guarantee" to "Extractors, formats, codecs and remote services can change or stop working. Zyvro does not guarantee continued compatibility.",
    ...
    LegalEntry("Disclaimer", "Content rights and software limitations", Icons.Rounded.Info, listOf(
        "Copyright" to "Downloading, converting or redistributing media may be restricted by copyright, licenses, platform terms, privacy rights or local law. Zyvro does not grant rights to media.",
        "Access controls" to "Do not use Zyvro to bypass protections or access controls you are not authorized to bypass.",
    ...
    LegalEntry("Copyright & DMCA", "Project and content ownership", Icons.Rounded.Copyright, listOf(
        "Project" to "Zyvro source is distributed under GPL-3.0-or-later unless a file states otherwise. Third-party notices remain applicable.",
        "Media" to "Zyvro does not own media, thumbnails, metadata or other content returned by remote services. Rights remain with their respective owners.",
    ...
    LegalEntry("Open Source Licenses", "Third-party software notices", Icons.Rounded.Code, listOf(
        "Zyvro" to "GPL-3.0-or-later. See the repository LICENSE file for the complete license text.",
...
fun LegalScreen(onBack: () -> Unit) {
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Spacer(Modifier.width(14.dp)); Column { Text("Legal & Open Source", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.ExtraBold); Text("Zyvro • v3.0.0", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
        }
